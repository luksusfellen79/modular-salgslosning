...
const LARVIK_FALLBACK = [
  { osm_id: 'f1', name: 'Glassmesteren', address: 'Torget 2, Larvik', lat: 59.0528, lng: 10.0266 },
  { osm_id: 'f2', name: 'Larvik Mikrobryggeri', address: 'Storgata, Larvik', lat: 59.0545, lng: 10.0260 },
  { osm_id: 'f3', name: 'Hansemann', address: 'Kongegata 33, Larvik', lat: 59.0538, lng: 10.0255 },
]
  ...
  for (const server of OVERPASS_SERVERS) {
    try {
      ...
      return data.elements
        .filter(el => el.tags?.name && el.tags.name.toLowerCase() !== 'larvik')
        .map(el => ({
          osm_id: el.id,
          name: el.tags.name,
...
export default function HostSetup() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      for (const { venue, order, activity } of selected) {
