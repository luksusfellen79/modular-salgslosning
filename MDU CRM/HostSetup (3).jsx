...
const LARVIK_FALLBACK = [
  { osm_id: 'f1', name: 'Storgata Pub', address: 'Storgata 31, Larvik', lat: 59.0559, lng: 10.0275 },
  { osm_id: 'f2', name: 'Larvik Mikrobryggeri', address: 'Storgata, Larvik', lat: 59.0555, lng: 10.0270 },
  { osm_id: 'f3', name: 'Old Irish Pub', address: 'Prinsegata 8, Larvik', lat: 59.0562, lng: 10.0265 },
  { osm_id: 'f4', name: 'Bazaar Bar', address: 'Storgata 24, Larvik', lat: 59.0558, lng: 10.0280 },
  { osm_id: 'f5', name: 'Bølgen & Moi', address: 'Nansetgata, Larvik', lat: 59.0550, lng: 10.0260 },
]

async function searchBarsOverpass(lat, lng, radius) {
  const query = `[out:json][timeout:25];(node["amenity"="bar"](around:${radius},${lat},${lng});node["amenity"="pub"](around:${radius},${lat},${lng});node["amenity"="nightclub"](around:${radius},${lat},${lng}););out body;`
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        // Slå sammen OSM-resultater med fallback, unngå duplikater på navn
        const osmNames = new Set(results.map(r => r.name.toLowerCase()))
        const extras = LARVIK_FALLBACK.filter(f => !osmNames.has(f.name.toLowerCase()))
        setVenues([...results, ...extras])
      } catch (e) {
        // Overpass feilet – bruk fallback
        setVenues(LARVIK_FALLBACK)
      } finally {
        setLoading(false)
      }
    }
